using ChemStats, DataFrames, CSV
...
pathin_fint = "C:\\Users\\Lisa\\OneDrive - UvA\\Minor data\\Serum data\\Clean spectra\\clean_fint" # files with feature's intensity
filename_fint = readdir(pathin_fint, join = true)

rep02 = CSV.read("C:\\Users\\Lisa\\OneDrive - UvA\\Minor data\\Serum data\\Clean spectra\\170426_02_Edith_120417_CCF_16_Cent_report_comp.CSV", DataFrame)
rep03= CSV.read("C:\\Users\\Lisa\\OneDrive - UvA\\Minor data\\Serum data\\Clean spectra\\170426_03_Edith_120417_CCF_17_Cent_report_comp.CSV", DataFrame)
rep14= CSV.read("C:\\Users\\Lisa\\OneDrive - UvA\\Minor data\\Serum data\\Clean spectra\\170425_14_Edith_120417_CCF_14_Cent_report_comp.CSV", DataFrame)
...
function changedf(num)
    ...
    df_fint = CSV.read(filename_fint[num],DataFrame)
    ...
    r_fint2= df_fint[:,11]
    df = innerjoin(r_mz1, r_int1, r_fint1, r_mz2, r_int2, r_fint2, on = :Nr, makeunique=true)
...
#feature alignment between runs - removing empty cells and non common features
...
filename_feat= readdir(pathin_feat, join = true)
    ...
    CSV.write("C:\\Users\\Lisa\\OneDrive - UvA\\Minor data\\Serum data\\Clean spectra\\spect align comp\\acc_20.csv",newdata)

#recombining replicates - consensus spectra
    function data1(rep)
    combinespect = DataFrame()
...
#acc 20, 40, 60 with 60windows
    ...
    acum_common = plot(acums, frags_win60, seriestype = :scatter, xlabel = "Acumulation Time", ylabel= "Number of Fragments of Common Feature", legend = false, title = "SWATH Windows 60")
    acum_plot = display("image/png", acum_common)
...
#acc 20, 40, 60 with 80windows
    ...
    acum_common = plot(acums, frags_win80, seriestype = :scatter, xlabel = "Acumulation Time", ylabel= "Number of Fragments of Common Feature", legend = false, title = "SWATH Windows 80")
    acum_plot = display("image/png", acum_common)
 ...
 #acc 20, 40, 60 with 100windows
    ...
    acum_common = plot(acums, frags_win100, seriestype = :scatter, xlabel = "Acumulation Time", ylabel= "Number of Fragments of Common Feature", legend = false, title = "SWATH Windows 100")
    acum_plot = display("image/png", acum_common)
 ...
 #win 60, 80, 100 with 20ms
    ...
    acum_common = plot(wins, frags_acc20, seriestype = :scatter, xlabel = "Acumulation Time", ylabel= "Number of Fragments of Common Feature", legend = false, title = "Accumulation Time 20")
    acum_plot = display("image/png", acum_common)
    ...
    acum_common = plot(wins, frags_acc40, seriestype = :scatter, xlabel = "Number of Windows", ylabel= "Number of Fragments of Common Feature", legend = false, title = "Accumulation Time 40")
    acum_plot = display("image/png", acum_common)
...
#Bar plots
p1 = bar([(20,1476), (40,2911),(60,1437)],xticks = 20:20:60, fill=["pink", "lightblue", "lightgreen"], legend = false, ylabel = "Total Number of Fragments", title="60")
p2 = bar([(20,432), (40,769),(60,698)], xticks = 20:20:60, fill=["pink", "lightblue", "lightgreen"], legend = false, title="80", ms=2, ma=0.5, xlabel = "Acumulation Time (ms)")
p3 = bar([(20,2222), (40,2013),(60,1945)], xticks = 20:20:60,fill=["pink", "lightblue", "lightgreen"],legend = false, title="100")
display("image/png", plot(p1,p2,p3,layout = (1,3)))
plot_title=("Number of SWATH Windows",plot_titlevspan=0.1)
#plot_title="Accumulation Time",plot_titlevspan=0.1
xticks!(0:20:60)

pa = bar([(60,1590), (80,1647),(100,3147)],xticks = 60:20:100, fill=["pink", "lightblue", "lightgreen"], legend = false, ylabel = "Total Number of Fragments", title="20ms")
pb= bar([(60,1015), (80,1005),(100,931)], xticks = 60:20:100, fill=["pink", "lightblue", "lightgreen"], legend = false, title="40ms", ms=2, ma=0.5, xlabel = "Number of SWATH Windows")
pc = bar([(60,1054), (80,1992),(100,1878)], xticks = 60:20:100,fill=["pink", "lightblue", "lightgreen"],legend = false, title="60ms")
